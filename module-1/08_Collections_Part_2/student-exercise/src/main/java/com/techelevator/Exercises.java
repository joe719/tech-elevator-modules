...
public class Exercises {
	...
	public double isItOnSale(String itemNumber) {
		...
		if(saleList.containsKey(itemNumber.toUpperCase())) {
			return saleList.get(itemNumber.toUpperCase());
		}
		else {
			return 0.00;
		}
...
//		
//		
//		if(itemNumber.contains("") || saleList.getValue) {
//			
//			return 0.00;
//		}
//		if()
	...
	public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
		...
		Integer peterMoney = peterPaul.get("Peter");
		Integer paulMoney = peterPaul.get("Paul");
		Integer paulFund = paulMoney/4;
		Integer peterFund = peterMoney/4;
		Integer peterPaulPartnership = peterFund + paulFund; 
		
		if(paulMoney >= 10000 && peterMoney >= 5000) {

			paulMoney -= paulFund;
			peterMoney -= peterFund;
	...
	Map<String, String> mapWithLetters = new HashMap<String, String>();
		
	String keyFirstLetter = "";
	String valueLastLetter = "";
	
	for(String word : words) {
		keyFirstLetter = word.substring(0,1);
		valueLastLetter = word.substring(word.length()-1);
		
		mapWithLetters.put(keyFirstLetter,valueLastLetter);
	}
	...
	public Map<String, Integer> consolidateInventory(Map<String, Integer> mainWarehouse,
		...
		for(String key : mainWarehouse.keySet()) {
			Integer value = mainWarehouse.get(key);
			
			output.put(key, value);
		}
		
		for(String key : remoteWarehouse.keySet()) {
			Integer value = remoteWarehouse.get(key);
			
			if(output.containsKey(key)) {
				Integer numberSoFar = output.get(key);
				Integer total = numberSoFar + value;
