...
public class Exercises {
	...
	public int max1020(int a, int b) {
		boolean biggerA = (a >= 10 && a <= 21); 
		boolean biggerB = (b >= 10 && b <= 21);
	...
	public boolean squirrelPlay(int temp, boolean isSummer) {
		if(temp <= 90 && temp >= 60 && !isSummer) {
			return true;
		} else if(temp <= 100 && temp >= 60 && isSummer){
			return true;
		} else {
		return false;
		}
    ...
    public String yourCakeAndEatItToo(double mealAmount, boolean isBirthday) {
    	if(mealAmount >= 11.00 && isBirthday) {
    		return "ginormous";
    	}
    	if(mealAmount <= 10.00 && mealAmount >= 5.00 && isBirthday) {
    		return "special";
    	}
    	if(mealAmount <= 5.00 && isBirthday) {
    		return "standard";
    	}
    	if(mealAmount <= 10.00 && !isBirthday) {
    		return "standard";
    	}
	...
	public boolean in1To10(int n, boolean outsideMode) {
		boolean inside = (n <= 10 && n >= 1);
		boolean outside = (n >= 10 && n <= 1);
		
		if(inside && !outsideMode) {
			return true;
		}
		if(outside && outsideMode) {
			return true;
		}
		if(inside && outsideMode) {
			return true;
